...
upload.addEventListener("change", function () {
  ...
  // ==== KONFIGURASI SESUAI TEMPLATE TWIBBON ====
  const circleCenterX = 384;     // posisi X tengah lingkaran (dalam px)
  const circleCenterY = 255;     // posisi Y tengah lingkaran
  const circleRadius = 185;      // jari-jari lingkaran
  ...
  // Pakai skala paling besar agar gambar MENUTUP LINGKARAN SEPENUHNYA
  imgScale = Math.max(scaleX, scaleY) * 1.05; // zoom 5% ekstra

  const newWidth = userImage.width * imgScale;
  const newHeight = userImage.height * imgScale;

  // ==== POSISI FOTO DI TENGAH LINGKARAN ====
  imgX = circleCenterX - newWidth / 2;
  imgY = circleCenterY - newHeight / 2 + 20; // offset wajah turun sedikit
