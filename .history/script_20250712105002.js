...
upload.addEventListener("change", function () {
    ...
    reader.onload = function (e) {
        ...
        userImage.onload = function () {
            ...
            // Hitung rasio cover (menutupi penuh area lingkaran)
            const scaleX = targetSize / userImage.width;
            const scaleY = targetSize / userImage.height;
            imgScale = Math.max(scaleX, scaleY) * 1.3; // Perbesar lagi agar benar-benar penuh

            const newWidth = userImage.width * imgScale;
            const newHeight = userImage.height * imgScale;

            // Atur posisi agar wajah pas di tengah & agak turun
            imgX = circleCenterX - newWidth / 2;
            imgY = circleCenterY - newHeight / 2 + 50; // Tambah offset ke bawah
